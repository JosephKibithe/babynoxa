...
export function WalletButton(){const wallet=useWallet();if(wallet.status==="unsupported")return <Button onClick={()=>void wallet.switchToSupported()}>Switch network</Button>;return <Button className="wallet-button" onClick={()=>void wallet.connect()}><Icon name="wallet"/><span>{wallet.address?short(wallet.address):wallet.status==="connecting"?"Connecting…":"Connect wallet"}</span></Button>;}
