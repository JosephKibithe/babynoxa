...
Canonical metadata JSON is stored through a `StorageAdapter`, immediately retrieved, and hash-verified before the service returns `metadataUri` and `metadataHash`. Image URL and website are optional; when an image is supplied it is validated and `imageHash` is also returned. A storage mismatch fails closed.
