...
/// @title BabyNoxaFactoryQS
/// @notice Deploys, funds, opens, and permanently registers one complete BabyNoxa launch atomically,
///         graduating onto a stock (permissionless) Uniswap-V2 factory such as QuickSwap.
/// @dev This is the QuickSwap sibling of BabyNoxaFactory. It creates the official pair with the stock
///      2-argument `createPair`, and therefore cannot rely on the guarded bootstrap-manager binding.
///      The pair is created empty at launch; GraduationManagerV2 defends the first liquidity seed.
contract BabyNoxaFactoryQS is IBabyNoxaFactory, Ownable2Step, ReentrancyGuard {
    ...
    constructor(
        ...
        // A stock QuickSwap factory has no launchFactory()/feeTo() coupling to assert; only require live code.
        if (v2Factory_.code.length == 0 || wrappedNative_.code.length == 0) revert InvalidV2Configuration();
        ...
        returns (LaunchRecord memory record)
    {
        ...
        // The token address only exists after this call, so no actor can front-run createPair in this tx.
        address officialPair = IV2Factory(v2Factory).createPair(address(token), wrappedNative);
        _validateNewPair(address(token), officialPair);
    ...
    function _validateNewPair(address token, address officialPair) private view {
        if (
            officialPair == address(0) || officialPair.code.length == 0
                || IV2Factory(v2Factory).getPair(token, wrappedNative) != officialPair
        ) revert InvalidOfficialPair();

        IQuickSwapV2Pair pair = IQuickSwapV2Pair(officialPair);
