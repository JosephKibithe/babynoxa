...
import {IV2Factory} from "./interfaces/dex/IV2Factory.sol";
import {IQuickSwapV2Pair} from "./interfaces/dex/IQuickSwapV2Pair.sol";
...
/// @notice Atomically converts one completed BabyNoxa curve into permanently locked QuickSwap-V2 liquidity.
/// @dev Unlike the guarded V1 manager, this targets a stock (permissionless) Uniswap-V2 factory such as
///      QuickSwap so graduated tokens are discoverable on QuickSwap/DexScreener. Because the pair is a
///      standard V2 pair, the atomic first-liquidity guarantee is replaced with a defensive seed:
///      the manager `skim`s any donated balances to the burn address, requires the pair to hold no
///      pre-existing liquidity (zero reserves, zero LP supply), mints all usable LP straight to the
///      burn address, and enforces the same terminal price-continuity band as V1. A griefer can still
///      brick a single launch's graduation by permanently locking synced reserves into the pair before
///      the completing buy; this is the accepted tradeoff of listing on a permissionless AMM.
contract GraduationManagerV2 is IGraduationManager, ReentrancyGuard {
    ...
    constructor(address factory_, address v2Factory_, address router_, address wrappedNative_) {
        ...
        // A stock QuickSwap factory has no launchFactory()/feeTo() coupling; we only pin the router topology.
        if (IV2Router02(router_).factory() != v2Factory_ || IV2Router02(router_).WETH() != wrappedNative_) {
            revert InvalidDeploymentConfiguration();
        }
        ...
        returns (GraduationResult memory result)
    {
        ...
        IQuickSwapV2Pair pair = _validatedPair(params);
        graduatedCurve[msg.sender] = true;

        // Flush any donated balances sitting on the empty pair into the burn address so the seed
        // reflects exactly the graduation liquidity. Reserves are guaranteed zero by _validatedPair.
        pair.skim(burnAddress);
        ...
        // Standard V2 seed: transfer both assets to the pair, then mint every usable LP to the burn address.
        IERC20(params.token).safeTransfer(params.officialPair, liquidityTokens);
        IERC20(wrappedNative).safeTransfer(params.officialPair, fee.liquidityBase);
        uint256 burnedLp = pair.mint(burnAddress);

        _validateMintResult(pair, params, fee.liquidityBase, liquidityTokens, burnedLp);
    ...
    function _validatedPair(GraduationParams calldata params) private view returns (IQuickSwapV2Pair pair) {
        address expectedPair = IV2Factory(v2Factory).getPair(params.token, wrappedNative);
        if (expectedPair != params.officialPair) revert InvalidOfficialPair(expectedPair, params.officialPair);

        pair = IQuickSwapV2Pair(params.officialPair);
        address token0 = pair.token0();
        address token1 = pair.token1();
        (uint112 reserve0, uint112 reserve1,) = pair.getReserves();
        bool assetsMatch =
            (token0 == params.token && token1 == wrappedNative) || (token0 == wrappedNative && token1 == params.token);
        // The pair must carry no pre-existing liquidity. A stock V2 pair cannot un-sync poisoned reserves,
        // so a nonzero reserve (attacker donation + sync) or existing LP supply is a hard, un-recoverable stop.
        if (!assetsMatch || pair.factory() != v2Factory || pair.totalSupply() != 0 || reserve0 != 0 || reserve1 != 0) {
            revert PairNotEmpty();
        }
    }

    function _orderedAmounts(IQuickSwapV2Pair pair, address token, uint256 tokenAmount, uint256 baseAmount)
        private
        view
    ...
    function _validateMintResult(
        IQuickSwapV2Pair pair,
        ...
        if (
            reserve0 != expected0 || reserve1 != expected1 || burnedLp == 0 || pair.balanceOf(burnAddress) != burnedLp
    ...
    function _validatePriceContinuity(IQuickSwapV2Pair pair, GraduationParams calldata params) private view {
        (uint112 reserve0, uint112 reserve1,) = pair.getReserves();
        uint256 poolTokens;
        uint256 poolBase;
